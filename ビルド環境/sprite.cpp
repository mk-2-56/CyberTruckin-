...
void SetSpriteColorRotation(ID3D11Buffer *buf, float X, float Y, float Width, float Height,
	float U, float V, float UW, float VH,
	XMFLOAT4 Color, float Rot, BOOL isTurn)					// isTuen:反転フラグ(省略すればfalse)
{
	D3D11_MAPPED_SUBRESOURCE msr;
	GetDeviceContext()->Map(buf, 0, D3D11_MAP_WRITE_DISCARD, 0, &msr);
	...
	// isTurnがTRUEの時は画像を反転させる
	vertex[0].TexCoord = XMFLOAT2((isTurn ? U + UW : U), V);
	vertex[1].TexCoord = XMFLOAT2((isTurn ? U : U + UW), V);
	vertex[2].TexCoord = XMFLOAT2((isTurn ? U + UW : U), V + VH);
	vertex[3].TexCoord = XMFLOAT2((isTurn ? U : U + UW), V + VH);
